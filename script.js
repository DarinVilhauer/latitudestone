...
(() => {
  ...
  const pct=n=>{if(!Number.isFinite(n))return '—';const v=Number(n);return (Number.isInteger(v)?v.toFixed(0):v.toFixed(2).replace(/0+$/,'').replace(/\.$/,''))+'%'};
  function reportData(type){
    ...
    const r=calculateInvestment();
    return {title:'Investment Analysis Summary',sections:[['Property Overview',[['Purchase Price',r.price?fmtCurrency(r.price):'—'],['Annual NOI',r.noi?fmtCurrency(r.noi):'—'],['Purchase Cap Rate',Number.isFinite(r.cap)?pct(r.cap*100):'—']]],['Financing Assumptions',[['LTV',pct(r.ltvPct)],['Down Payment',pct(r.downPct)],['Interest Rate',pct(r.ratePct)],['Amortization',(r.amortYears||25)+' years']]],['Operating Assumptions',[['Vacancy',pct(r.vacancyPct)],['Reserves',pct(r.reservesPct)],['Management',pct(r.managementPct)] ]],['Calculation Results',[['Adjusted NOI',Number.isFinite(r.adjustedNoi)?fmtCurrency(r.adjustedNoi):'—'],['Annual Debt Service',Number.isFinite(r.annualDebt)?fmtCurrency(r.annualDebt):'—'],['Annual Cash Flow',Number.isFinite(r.cashflow)?fmtCurrency(r.cashflow):'—'],['Loan Amount',Number.isFinite(r.loan)?fmtCurrency(r.loan):'—'],['Down Payment',Number.isFinite(r.equity)?fmtCurrency(r.equity):'—'],['DSCR',Number.isFinite(r.dscr)&&r.annualDebt>0?r.dscr.toFixed(2)+'x':'—']]]],disclaimer:'For informational and preliminary analysis purposes only. Results are estimates based on user-provided information and assumptions and are not financial, lending, tax, legal, appraisal, or investment advice. Actual financing terms, lender requirements, expenses, and investment performance may vary. Users should independently verify all information before making a real estate or financing decision.'};
  }
  ...
  $('results-print-confirm')?.addEventListener('click',()=>{const html=reportHTML(exportType);const w=window.open('','_blank','width=900,height=1100');if(!w)return;w.document.write(`<!doctype html><html><head><title>Latitude Stone Results</title><style>body{font-family:Arial,sans-serif;margin:0;padding:32px;color:#17202a}.report-sheet{max-width:760px;min-height:920px;margin:auto}.report-header{display:flex;justify-content:space-between;gap:30px;padding-bottom:24px;border-bottom:1px solid #d9d2c3}.report-brand{text-align:left}.report-brand strong{display:block;letter-spacing:4px;font-size:18px;font-weight:500}.report-brand span{display:block;letter-spacing:3px;font-size:8px;color:#8b7248;margin-top:4px}.report-title{text-align:right;font-size:11px}.report-title strong{display:block;font-size:14px}.report-section{margin-top:46px}.report-section h4{font-size:12px;margin:0 0 12px;padding-bottom:8px;border-bottom:1px solid #ddd}.report-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px 22px}.report-item span{display:block;color:#69737c;font-size:9px}.report-item strong{display:block;margin-top:4px;font-size:12px}.report-disclaimer{margin-top:48px;padding-top:18px;border-top:1px solid #ddd;color:#69737c;font-size:8px;line-height:1.55}@media print{body{padding:0}.report-sheet{min-height:0}}</style></head><body>${html}<script>window.onload=()=>setTimeout(()=>window.print(),200)<\/script></body></html>`);w.document.close();});
